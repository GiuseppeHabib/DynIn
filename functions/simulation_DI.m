function [xt,t,cell_f,tipo,new_solution_c,new_solution_p,count]=simulation_DI(dim,par,x0,tfinal,Ts,option,spaceboundary,discr,cell_c,cell_d,cell_dout ...
...
while converged<0.5 % stop if converged=1
    ...
    for i=1:dim
        if (xt(count,i)>spaceboundary(i) && xt(count,i)<spaceboundary(dim+i)) % check if the point is withing the phase space
            if isempty(find(cell_pos(:,i)>=xt(count,i)-dh(i)/2 & cell_pos(:,i)<=xt(count,i)+dh(i)/2,1))
                ...
                if count>3 % checking only after 3 steps at least
                    ...
                    if ~isempty(temp_selfconv) % if there are recurrent cells, then goes on checking, otherwise it stopd this search and simulate another point
                        if length(temp_selfconv)>rep_fix_point % if the cell was repeated more than "rep_fix_point" times then it is assumed that we have a new fixed point, tipo=4
                            converged=1;
                            tipo=4;
                            new_solution_c=cell_f(count); % store the cell of the new fixed point
