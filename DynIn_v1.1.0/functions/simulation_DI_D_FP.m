function [xt,t,cell_f,tipo,new_solution_p,count]=simulation_DI_D_FP(dim,par,x0,tfinal,Ts,tols,spaceboundary,discr,cell_c,cell_d,cell_dout, ...
...
while converged<0.5 % stop if converged=1
    ...
    if count == 2 % the first round is with the xt initial conditions
        init = 1;
        [x,zsemidisc] = ddemap(xt(count-1,:),par,dim,init);
        init = 0;
    else % the further rounds are initiated with the output of the semidiscretization
        ...
        if ~isempty(temp_c) % if it does correspond to a converging cell (temp_c not empty) then tipo=1 and converged
            ...
            if tipo==0 % last considered case, is the case of a new solution, previously unknown
                if count>3 % checking only after 3 steps at least
                    ...
                    if ~isempty(temp_selfconv) % if there are recurrent cells, then goes on checking, otherwise it stopd this search and simulate another point
