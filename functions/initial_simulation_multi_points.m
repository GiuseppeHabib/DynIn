function xe=initial_simulation_multi_points(dim,par,varargin)
% initial_simulation_multi_points(dim,par,varargin)
%
% perform a simulation of the system, by default with zero vector initial
% conditions, looking for the periodic solution.
% it provides in output n points correspondign to the point on the 
% stroboscopic poincaré section of the system, where n is the number of
% section to be taken in one period.
% n is defined by the entry "points"
...
% frequency in rad/s
% points: number of point to discretize the periodic solution (default value
% 15)
...
[~,xe]=ode45 (@(t,xt) sistema(t,xt,par), [0:Per/(points-1):Per], x0, option); % single simulation of one period length for obatining he periodic solution
if plot_period==1
    figure;plot(xe(:,var1),xe(:,var2),'go','LineWidth',2);
end
