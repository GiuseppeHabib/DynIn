function [R_final,OutL,OutH]=compute_LIM_D_LCO(xe0,par,varargin)
...
% habib@mm.bme.hu or szaksz@mm.bme.hu.
...
dim=size(xe0,2); % dimension of the reduced system
...
DEFNper_dense=1001; % define the number of points within the discretized periodic orbit
DEFTs=par.tau; % define the sampling time as the time delay
...
DEFstab_fixedpoint=[]; % define the Poincaré section: variable and value
...
addOptional(p,'Nper_dense',DEFNper_dense);
...
addOptional(p,'stab_fixedpoint',DEFstab_fixedpoint);
...
if check_per==1
    ...
    sol_new=check_per_orbit_D_LCO(tfinal,Tmax,xe0,par,tols,Poincare_sec,number_of_steps,dim,flag_init,event_on);
    sol_check_per_all = sol_new;
    Z = (sol_new.y).';
    x(1,:)=Z(end,:);
    err=10e9;
    flag_init = 0;
    % simulate the trajectories until it converges
    while err > abstol*1000
        sol_prev = sol_new;
        sol_prev.x = sol_prev.x-sol_prev.x(end);
        sol_new=check_per_orbit_D_LCO(tfinal,Tmax,sol_prev,par,tols,Poincare_sec,number_of_steps,dim,flag_init,event_on);
        Z = (sol_new.y).';
        J=J+1;
        if J>1000
        ...
        sol_check_per_all.x = [sol_check_per_all.x, sol_check_per_all.x(end) + sol_new.x(2:end)]; % save the whole initial trajectory to use it later
        sol_check_per_all.y = [sol_check_per_all.y, sol_new.y(:,2:end)];
        sol_check_per_all.yp = [sol_check_per_all.yp, sol_new.yp(:,2:end)];
        x(J+1,:)=Z(end,:);
    end
...
%     tvec=t0;
    ...
    %Z=zeros(size(Z));
    %timeser=0;
    while err > abstol*10000 % here, it should be larger than abstol, otherwise the period may be multiple
        sol_prev = sol_new;
        sol_prev.x = sol_prev.x-sol_prev.x(end);
        sol_new=check_per_orbit_D_LCO(tfinal,Tmax,sol_prev,par,tols,Poincare_sec,number_of_steps,dim,flag_init,event_on);
        if J == 0
            ...
            sol_per.x = [sol_per.x, sol_per.x(end)+sol_new.x(2:end)];
            sol_per.y = [sol_per.y, sol_new.y(:,2:end)];
            sol_per.yp = [sol_per.yp, sol_new.yp(:,2:end)];
            Z2 = (sol_new.y(:,2:end)).';  
        end      
        %Z(timeser+1:size(Z2,1),:)=Z2;
        %timeser=size(Z2,1);
        J=J+1;
        if J>100
        ...
        err = min( err , max( abs(Z2(end,:)-x(1,:))) );
    end

     
    Tper = sol_per.x(end);
    sol_per.x = sol_per.x-sol_per.x(end);
    %Tmax = [0,(ceil(par.tau/Tper)+1)*Tper];
    Tmax = (ceil(par.tau/Tper)+1)*Tper;
    event_on = 0;
    sol_per_ext=check_per_orbit_D_LCO(tfinal,Tmax,sol_per,par,tols,Poincare_sec,number_of_steps,dim,flag_init,event_on);
    sol_per_ext.x = sol_per_ext.x-ceil(par.tau/Tper)*Tper;
...
if OutL.flag_NaN==1
        ...
        OutL.x0=NaN; OutL.point_crit=NaN; OutL.xe=NaN;
    return
end

sol_check_per_all.x = [sol_check_per_all.x, sol_check_per_all.x(end) + sol_per.x(2:end)]-ceil(par.tau/Tper)*Tper; % save the whole initial trajectory to use it later
sol_check_per_all.y = [sol_check_per_all.y, sol_per.y(:,2:end)];
sol_check_per_all.yp = [sol_check_per_all.yp, sol_per.yp(:,2:end)];
   
if max(max(abs(sol_per_ext.y))) <abstol*10000
    OutL.flag_NaN=1;
    OutH = [];
    R_final = nan;
    return
...
% extended solution, which contains the history also for the first point of the periodic solution
OutL.solper = sol_per_ext;
timevec_xe = linspace(0,sol_per_ext.x(end),Nt+1);
timevec_xe = timevec_xe(2:end);
timevec_per_dense = linspace(0,sol_per_ext.x(end),Nper_dense);
timevec_check_per_dense = linspace(0,sol_check_per_all.x(end),round(sol_check_per_all.x(end)/Tper)*Nper_dense);
timevec_disc_xe = flip(linspace(-par.tau,0,par.N));

% sampled initial points 
for ixe = 1 : size(timevec_xe,2)
    xe_i_matr= deval(sol_per_ext,timevec_xe(ixe)+timevec_disc_xe);
    OutL.xe(ixe,:) = reshape(xe_i_matr,1,dim);
end
% periodic solution in the extended space
Z=zeros(Nper_dense,dim);
for iper = 1 : size(timevec_per_dense,2)
    per_i_matr= deval(sol_per_ext,timevec_per_dense(iper)+timevec_disc_xe);
    Z(iper,:) = reshape(per_i_matr,1,dim);
end
% points converging to the periodic solution in the extended space
Z_conv=zeros(Nper_dense,dim);
for iper = 1 : size(timevec_check_per_dense,2)
    per_i_matr= deval(sol_check_per_all,timevec_check_per_dense(iper)+timevec_disc_xe);
    Z_conv(iper,:) = reshape(per_i_matr,1,dim);
end
...
minloc=minimum;
i1=1;
for i=1:dim % look for the minimal distance between the periodic solution and phase space boundary
    for j = 1:size(Z,1)
...
if ~isempty(stab_fixedpoint) % reduce R according to the minimal distance between the periodic solution and the provided stable fixed point
    min_dist_xe = min(distance_from_xe(Z,stab_fixedpoint,weight));
    OutL.R(1)= min([OutL.R(1),min_dist_xe]);
end
...
% find cells of trajectory converging to periodic orbit
cell_e_init=zeros(dim,size(Z_conv,1)); % inizialize cell of values of the periodic orbit
for i=1:dim % for each coordinate, look for the corresponding cell index
    for j = 1:size(Z_conv,1)
        [~,cell_e_init(i,j)]=min(abs(cell_pos(:,i)-Z_conv(j,i)));
    end
end

cell_f_around_eq = zeros(num_of_cell_around_equilibrium^dim-1,size(Z_conv,1));
for i = 1:size(Z_conv,1)
    OutH.vc(ind_cp+1,:)=Z_conv(i,:); % assign the converging initial trajectory to the converging points
    ind_cp=ind_cp+1; % update counter of converging points
    cell_c(ind_cc+1)=find_index(cell_e_init(:,i),dim,discr); % assign to the converging cells the cell containing xe (here ind_cc=0)
    ind_cc=ind_cc+1; % update counter of converging points
    cell_f_around_eq(:,i)=find_cells_around_a_cell(dim,cell_e_init(:,i),discr,num_of_cell_around_equilibrium); % find cells around equilibrium cell which are then assigned as converging
...
% index of points along the periodic solution with which a bisection method
% using the "origin" (middle point) as the headpoint
x0_ind_HP_origin = 1;

OutL.xe_ind(1) = x0_ind_HP_origin;
OutL.x0(1,:) = OutL.xe(OutL.xe_ind(1),:); 
if ~isempty(stab_fixedpoint)
    OutL.x0(1,1:dim_df)=stab_fixedpoint(:,1:dim_df); % first initial condition chosen by the stab_fixedpoint
else
    OutL.x0(1,1:dim_df)=mean(OutL.xe(:,1:dim_df)); % first initial condition chosen by the stab_fixedpoint
end
sol_init = sol_per_ext;
for i1=1:number_of_steps % cycle over the pre-defined number of steps
    ...
    %%
    sol_init.x = sol_per_ext.x-timevec_xe(OutL.xe_ind(i1)); % shift the time axis of the periodic solution to the actual point
    
    [xt,t,cell_f,OutL.tipo(i1),new_solution_p,OutL.time_steps_sim(i1)]=simulation_DI_D_LCO(dim,par,OutL.x0(i1,:),sol_init,tfinal,Ts,tols,spaceboundary,discr,cell_c(1:ind_cc,:), ...
        cell_d(1:ind_dc,:),cell_dout(1:ind_doutc,:),cell_pos,dh,rep_fix_point,rep_periodic,min_periodic_radius,check_min_periodic,weight,Z_conv);
        ...
        if type_x0==1
            ...
            for J=1:Nt
                ...
                if Dist > OutL.distance(i1+1)
                    OutL.distance(i1+1)=Dist; % this is not the final distance since the history is mapped to the periodic solution
                    OutL.x0(i1+1,1:dim_df)=OutL.xe(J,1:dim_df);
                    OutL.x0(i1+1,1:dim_df)=X0(1:dim_df);
                    OutL.xe_ind(i1+1) = J;
                end                
            end            
            OutL.time_x0(i1)=toc(x0_tic); % measure time for choosing new initial condition
            OutL.num_points_inside_LCO(i1,1)=ind_v_inside; % save the number of point inside the hypersphere of convergence at this step
        elseif type_x0==2
            ...
            if OutL.tipo(i1)<2 % if the last trajectory was converging
                ...
                if (count_bis<=bis_iter_max && there_was_div>0.5) % if the maximal number of bisection step is not reached AND there was already a diverging trajectory in the bisection procedure we continue the bisection
                    ...
                    OutL.xe_ind(i1+1) = OutL.xe_ind(i1);
                elseif ~isempty(stab_fixedpoint) && x0_ind_HP_origin+2 <= Nt
                    x0_ind_HP_origin = x0_ind_HP_origin+2; % every second point is checked with 0 as IC headpoint
                    OutL.xe_ind(i1+1) = x0_ind_HP_origin;
                    OutL.x0(i1+1,:) = OutL.xe(OutL.xe_ind(i1+1),:); 
                    OutL.x0(i1+1,1:dim_df)=0*mean(OutL.xe(:,1:dim_df)); %% !!!!!!!!!!!!!!!!!! 0*...
                else % if converging trajectory, but either maximal bisection steps number is passed OR there was no diverging trajectory OR all the xE points were used with the zero as initial condition, then we select a random initial condition and then restart the bisection
                    ...
                    OutL.xe_ind(i1+1) = randi(Nt);  
                    OutL.x0(i1+1,:)=randomIC_border(dim,OutL.R(i1+1)*1.5,OutL.xe(OutL.xe_ind(i1+1),:),weight); % find new initial condition randomly on the boundary of the hypersphere of convergence with a slightly larger radius  
                    OutL.x0(i1+1,dim_df+1:end) = OutL.xe(OutL.xe_ind(i1+1),dim_df+1:end);
                end
                ...
                if (ind_closest>1 || count_bis>bis_iter_max) % if the closest point to xe is not the first one OR we already had too many bisection steps we start a new bisection from the closest point
                    ...
                    OutL.x0(i1+1,:)=OutL.xe(Jcrit,:); % find next initial condition
                    OutL.x0(i1+1,1:dim_df)=lastX0div(1:dim_df);
                    OutL.xe_ind(i1+1) = Jcrit;
...
%             vcount_bis(i1)=count_bis; % save the counter of the bisection method for this step
        elseif type_x0==3
            ...
            OutL.xe_ind(i1+1) = randi(Nt);
            OutL.x0(i1+1,:)=randomIC_radius(dim,OutL.R(i1+1)*1.1,OutL.xe(OutL.xe_ind(i1+1),:),weight); % select the next initial condition within the hypersphere of convergence (with enlarged radius)
            OutL.x0(i1+1,dim_df+1:end) = OutL.xe(OutL.xe_ind(i1+1),dim_df+1:end);
            % check if the initial condition is inside the phase space
            out_bound=0;
            for i=1:dim
            ...
            if out_bound>0.5
                OutL.x0(i1+1,:)=randomIC_border(dim,OutL.R(i1+1),OutL.xe(OutL.xe_ind(i1+1),:),weight); % if this is the case, select a new initial point, this time exactly on the hypersphere of convergence, to be sure it is not outside
                OutL.x0(i1+1,dim_df+1:end) = OutL.xe(OutL.xe_ind(i1+1),dim_df+1:end);
            end
        end
        if plot_results>0.5 % if chosen to plot at every step
