function out_bound = checkoninput(dim,spaceboundary,xe,weight,var)
...
if var(1)>dim
    error('"var1" should be equal or less than the system dimension');
end
if var(2)>dim
    error('"var2" should be equal or less than the system dimension');
end
...
% checks whether the points of the solution are withing the examined phase space
out_bound=0;
for i=1:dim
    if (min(xe(:,i))<spaceboundary(i) || max(xe(:,i))>spaceboundary(dim+i)) 
        out_bound=1;
        break
    end
end
